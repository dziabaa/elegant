...
async fetch(req, env) {
const O = “*”;
const M = “GET,PUT,POST,DELETE,OPTIONS”;
...
async function getSess(r) {
  ...
  const s = ss[token];
  if (!s) return null;
  if (s.expires < Date.now()) { delete ss[token]; await kv.put("sessions", JSON.stringify(ss)); return null; }
  return s;
}
...
if (url.pathname === "/auth/login" && req.method === "POST") {
  ...
  if (!ul.length) {
    ul = [{ id: 1, login: "admin", passHash: "8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918", role: "admin", name: "Administrator" }];
    await kv.put("users", JSON.stringify(ul));
  }
  const u = ul.find(x => x.login === login && x.passHash === ph);
...
if (url.pathname === "/auth/logout" && req.method === "POST") {
  const tok = req.headers.get("X-Auth-Token");
  if (tok) {
    const sr = await kv.get("sessions");
    const ss = sr ? JSON.parse(sr) : {};
    delete ss[tok];
...
if (url.pathname === "/users" && req.method === "POST") {
  if (!await needRole(req, ["admin"])) return fail("Forbidden", 403);
  const b = await req.json();
  if (!b.login || !b.passHash || !b.role) return fail("Missing fields");
  const ur = await kv.get("users");
  const ul = ur ? JSON.parse(ur) : [];
  if (ul.find(u => u.login === b.login.toLowerCase())) return fail("Login exists");
  const mid = ul.reduce((m, u) => Math.max(m, u.id), 0);
  const nu = { id: mid + 1, login: b.login.toLowerCase(), passHash: b.passHash, role: b.role, name: b.name || b.login };
  ul.push(nu);
  await kv.put("users", JSON.stringify(ul));
  return ok({ id: nu.id, login: nu.login, role: nu.role, name: nu.name });
}

if (url.pathname === "/users" && req.method === "PUT") {
  if (!await needRole(req, ["admin"])) return fail("Forbidden", 403);
  const uid = parseInt(url.searchParams.get("id"));
  const b = await req.json();
  const ur = await kv.get("users");
  const ul = ur ? JSON.parse(ur) : [];
  const idx = ul.findIndex(u => u.id === uid);
  if (idx === -1) return fail("Not found", 404);
  if (b.name !== undefined) ul[idx].name = b.name;
  if (b.role !== undefined) ul[idx].role = b.role;
  if (b.passHash) ul[idx].passHash = b.passHash;
  await kv.put("users", JSON.stringify(ul));
  return ok({ ok: true });
}
...
if (url.pathname === "/users" && req.method === "DELETE") {
  ...
  const ac = ul.filter(u => u.role === "admin").length;
  if (tgt && tgt.role === "admin" && ac <= 1) return fail("Cannot delete last admin");
  ul = ul.filter(u => u.id !== uid);
  await kv.put("users", JSON.stringify(ul));
  return ok({ ok: true });
}
...
if (url.pathname === "/data") {
  ...
  if (req.method === "GET") {
    ...
    return new Response(v || "null", { headers: cors() });
  }
  if (req.method === "PUT") {
    if (s.role === "cleaner" || s.role === "viewer") return fail("Forbidden", 403);
    await kv.put(key, await req.text());
    return ok({ ok: true });
  }
}
...
if (url.pathname === "/inquiry" && req.method === "POST") {
  const b = await req.json();
  const raw = await kv.get("inquiries");
  ...
  return ok({ ok: true, id: inq.id });
}

if (url.pathname === "/inquiry" && req.method === "GET") {
...
const resp = await fetch(target, { headers: { "User-Agent": "Mozilla/5.0", "Accept": "text/calendar,*/*" } });
const txt = await resp.text();
return new Response(txt, { status: resp.status, headers: {
  "Access-Control-Allow-Origin": O,
